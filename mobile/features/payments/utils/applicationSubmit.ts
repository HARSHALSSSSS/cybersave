...
import {
  PaymentSettledError,
  isRetriablePaymentError,
} from '@utils/paymentResilience';
...
/** Backend submit requires PAYMENT_PENDING + CAPTURED after a paid application. */
...
/** Razorpay capture can land a beat before submit accepts the application. */
...
/** Human-readable server reason, so a stuck submit is diagnosable instead of "payment failed". */
...
/**
 * Wait until the backend sees a captured payment, then submit with retries.
 * If the application was already submitted (e.g. lost response), return it.
 */
